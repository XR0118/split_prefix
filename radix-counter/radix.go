...
func (es edges) Iterator() func() (int, edge, bool) {
	index := 0
	return func() (idx int, val edge, ok bool) {
		if index >= len(es) {
			return
		}

		idx, val, ok = index, es[index], true
		index++
		return
	}
}
...
type FileFn func(prefix string) (map[string]int, error)
